...
const createUnit = async (req, res) => {
  const { unitCode, monthlyPrice, vacancyStatus, floorPlan, location, description, bedrooms } = req.body;
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO units (unit_code, monthly_price, vacancy_status, floor_plan, location, description, bedrooms, admin_id)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`,
      [unitCode.toUpperCase(), parseFloat(monthlyPrice), vacancyStatus || 'vacant', floorPlan || null, location || null, description || null, bedrooms || null, req.user.userId]
    );
  ...
  try {
    const result = await pool.query(
      `UPDATE units SET unit_code = COALESCE($1, unit_code), monthly_price = COALESCE($2, monthly_price),
       ...
       bedrooms = COALESCE($7, bedrooms), updated_at = NOW()
       WHERE unit_id = $8 RETURNING *`,
      [unitCode, monthlyPrice ? parseFloat(monthlyPrice) : null, vacancyStatus, floorPlan, location, description, bedrooms || null, id]
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ success: false, message: 'Unit not found.' });
    }
...
const deleteUnit = async (req, res) => {
  ...
  try {
    ...
    const unitCheck = await client.query(
      'SELECT unit_id, unit_code, vacancy_status FROM units WHERE unit_id = $1',
      [id]
    );
