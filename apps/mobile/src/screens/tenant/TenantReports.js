...
function StatCard({ label, value, accent }) {
  return (
    <View style={[s.statCard, accent && { backgroundColor: BLUE }]}>
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: COLORS.pageBg }} edges={['top']}>
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ padding: 16, gap: 14 }}>
        ...
        {report && (
          <>
            {/* Period header */}
            <View style={[s.periodHeader, { backgroundColor: BLUE }]}>
            ...
            {/* My Unit */}
            <View style={s.card}>
              <Text style={s.sectionLabel}>MY UNIT</Text>
              <View style={s.statRow}>
                <StatCard label="Unit" value={report.unit.unit_code} accent />
                <StatCard label="Monthly Rent" value={formatPeso(report.unit.monthly_rent)} />
                <StatCard label="Due Day" value={ordinal(report.unit.due_day)} />
              </View>
            </View>

            {/* My Payments */}
            <View style={s.card}>
              <Text style={s.sectionLabel}>MY PAYMENTS</Text>
              <Text style={s.bigAmount}>{formatPeso(report.payments.total_paid)}</Text>
              <View style={s.statRow}>
                <StatCard label="Transactions" value={report.payments.total_transactions} />
                <StatCard label="Late Payments" value={report.payments.late_payments} />
              </View>
              {report.payments.breakdown.length === 0 ? (
                ...
                <View style={{ marginTop: 14 }}>
                  <View style={s.tableHeader}>
                    {['Amount', 'Month', 'Status', 'Date'].map(h => (
                      <Text key={h} style={s.tableHeaderCell}>{h}</Text>
                    ))}
                  </View>
                  {report.payments.breakdown.map((p, i) => (
                    <View key={i} style={s.tableRow}>
                      <Text style={[s.tableCell, { color: BLUE, fontWeight: '700' }]}>{formatPeso(p.amount)}</Text>
                      <Text style={s.tableCell} numberOfLines={1}>{p.month_covered}</Text>
                      <View style={{ flex: 1 }}>
                        <StatusPill status={p.status} isLate={p.is_late} />
                      </View>
                      <Text style={[s.tableCell, s.tableCellMuted, { fontSize: 11 }]}>{fmtDate(p.date)}</Text>
                    </View>
                  ))}
                </View>
              )}
            ...
            {/* My Maintenance */}
            <View style={s.card}>
              <Text style={s.sectionLabel}>MY MAINTENANCE</Text>
              <View style={s.statRow}>
...
const s = StyleSheet.create({
  ...
  periodBtnActive: { borderColor: BLUE, backgroundColor: BLUE },
  ...
  generateBtn: {
    marginTop: 14, height: 46, borderRadius: 8, backgroundColor: BLUE,
  ...
  bigAmount: { fontSize: 26, fontWeight: '800', color: BLUE, marginBottom: 12 },
